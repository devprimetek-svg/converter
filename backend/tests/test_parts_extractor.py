...
def test_four_character_model_code_formats():
    """Verify that all 4-character model code formats are strictly recognized in both orientations:
    - BJPK (all letters)
    - BK2C (letters + digit + letter)
    - 34VD (digits + letters)
    - 3453 (pure 4 digits)
    - F54D (letter + digits + letter)
    """
    from app.parts_extractor import (
        is_valid_4char_model_code,
        resolve_model_code,
        detect_vertical_model_columns,
    )

    # 1. Validation test
    for code in ["BJPK", "BK2C", "34VD", "3453", "F54D", "BGPK"]:
        assert is_valid_4char_model_code(code) is True
        assert len(code) == 4

    # Non-4-char or invalid codes must be rejected
    for invalid in ["1", "12", "QTY", "REF", "PART", "PAGE", "2026", "BGPK1", ""]:
        assert is_valid_4char_model_code(invalid) is False

    # 2. Resolution test for all formats in both orientations
    assert resolve_model_code("BJPK", ["BJPK"]) == "BJPK"
    assert resolve_model_code("KPJB", ["BJPK"]) == "BJPK"
    assert resolve_model_code("BJPK", None) == "BJPK"

    assert resolve_model_code("BK2C", ["BK2C"]) == "BK2C"
    assert resolve_model_code("C2KB", ["BK2C"]) == "BK2C"
    assert resolve_model_code("BK2C", None) == "BK2C"

    assert resolve_model_code("34VD", ["34VD"]) == "34VD"
    assert resolve_model_code("DV43", ["34VD"]) == "34VD"
    assert resolve_model_code("34VD", None) == "34VD"

    assert resolve_model_code("3453", ["3453"]) == "3453"
    assert resolve_model_code("3543", ["3453"]) == "3453"
    assert resolve_model_code("3453", None) == "3453"

    assert resolve_model_code("F54D", ["F54D"]) == "F54D"
    assert resolve_model_code("D45F", ["F54D"]) == "F54D"
    assert resolve_model_code("F54D", None) == "F54D"

    # 3. Direct vertical detection test for 34VD and 3453
    # Top-to-bottom 3, 4, V, D
    words_34vd = [
        {"text": "3", "top": 60.0, "bottom": 67.0, "x0": 350.0, "x1": 357.0},
        {"text": "4", "top": 68.0, "bottom": 75.0, "x0": 350.1, "x1": 357.1},
        {"text": "V", "top": 76.0, "bottom": 83.0, "x0": 349.9, "x1": 356.9},
        {"text": "D", "top": 84.0, "bottom": 91.0, "x0": 350.0, "x1": 357.0},
    ]
    cols = detect_vertical_model_columns(
        words=words_34vd,
        header_top=85.0,
    ...
    assert len(cols) == 1
    assert cols[0]["name"] == "34VD"

    # Top-to-bottom 3, 4, 5, 3
    words_3453 = [
        {"text": "3", "top": 60.0, "bottom": 67.0, "x0": 350.0, "x1": 357.0},
        {"text": "4", "top": 68.0, "bottom": 75.0, "x0": 350.1, "x1": 357.1},
        {"text": "5", "top": 76.0, "bottom": 83.0, "x0": 349.9, "x1": 356.9},
        {"text": "3", "top": 84.0, "bottom": 91.0, "x0": 350.0, "x1": 357.0},
    ]
    cols_num = detect_vertical_model_columns(
        words=words_3453,
        header_top=85.0,
        header_bottom=95.0,
        desc_x1=300.0,
    ...
    assert len(cols_num) == 1
    assert cols_num[0]["name"] == "3453"


def test_synthetic_pdf_extraction_various_model_codes():
    """Create a synthetic PDF with 4-character model codes and verify Excel export headers."""
    from reportlab.pdfgen import canvas
    from reportlab.lib.pagesizes import letter
    from app.excel_export import generate_excel_workbook
    import openpyxl

    for model_code in ["BJPK", "BK2C", "34VD", "3453", "F54D"]:
        buf = io.BytesIO()
        c = canvas.Canvas(buf, pagesize=letter)
        # Cover page with parenthesized model code
        c.drawString(100, 700, f"PARTS CATALOGUE MODEL ({model_code}) INDIA")
        c.showPage()

        # Foreword
        c.drawString(100, 700, f"FOREWORD Model Code: {model_code}")
        c.showPage()
        ...
        c.drawString(120, 520, "PART NO.")
        c.drawString(240, 520, "DESCRIPTION")
        # Vertical characters for model_code between DESCRIPTION and REMARKS (x=520)
        for i, char in enumerate(model_code):
            c.drawString(520, 540 - (i * 7), char)
        c.drawString(600, 520, "REMARKS")
        # Parts row
        c.drawString(50, 490, "1")
        c.drawString(120, 490, "B7J-E1102-00")
        ...
        result = extract_parts_from_pdf(buf)
        assert model_code in result["model_columns"]
        assert len(result["rows"]) == 1
        assert result["rows"][0][model_code] == "1"

        # Verify Excel generation includes exactly that 4-character column
        excel_buf = generate_excel_workbook(result["rows"], result["model_columns"])
        wb = openpyxl.load_workbook(excel_buf)
        ws = wb.active
        headers = [cell.value for cell in ws[1]]
        assert model_code in headers
        # Check that header cell value length is exactly 4
        m_cell = next(h for h in headers if h == model_code)
        assert len(m_cell) == 4
